...
bool Sema::ActOnCXXNestedNameSpecifierUnrefltype(CXXScopeSpec &SS,
                                                 const DeclSpec &DS,
                                                 SourceLocation ColonColonLoc) {
  if (SS.isInvalid() || DS.getTypeSpecType() == DeclSpec::TST_error)
    return true;
  ...
  QualType T = BuildUnrefltypeType(DS.getRepAsExpr(), DS.getTypeSpecTypeLoc());
  if (!T->isDependentType() && !T->getAs<TagType>()) {
    Diag(DS.getTypeSpecTypeLoc(), diag::err_expected_class_or_namespace) 
      << T << getLangOpts().CPlusPlus;
    return true;
  ...
  TypeLocBuilder TLB;
  UnrefltypeTypeLoc UnrefltypeTL = TLB.push<UnrefltypeTypeLoc>(T);
  UnrefltypeTL.setNameLoc(DS.getTypeSpecTypeLoc());
  SS.Extend(Context, SourceLocation(), TLB.getTypeLocInContext(Context, T),
            ColonColonLoc);
  return false;
}
